    ...
    switch IFMethod
      ...
      case 'S'
        for i = 1:n
          ...
          [RwallEN, LwallEN] = movewall(MST, IFT, i, d);
...
%% Simplify
  ...
  function [RwallEN, LwallEN] = movewall(MST, IFT, idx, d)
    IF = IFT.get(idx);
    cID = IFT.getancestors(idx);
    cID = cID(end-1); %end = SC, end-1 = MS
    cMS = MST.get(cID);
    cIFT = IFT.subtree(cID,[],'match');
    bIFs = cIFT.getnear(idx-cID+1);
    if cIFT.get(idx-cID+1) ~= IF; warning('cIFT.getnear??'); end
    ...
    for pID = 1:size(IF.p,1)
      PnearBIF = [];
      for nBif = nearBIF
        if nBif.isin(IF.p(pID,:))
          PnearBIF = [PnearBIF, nBif];
        end
      end
      PnearSL = [];
      for fi = 1:numel(PnearBIF)
        for fj = fi+1:numel(PnearBIF)
          PnearSL = [PnearSL,PnearBIF(fi).FxF(PnearBIF(fj))];
        end
      end
      moveR = [];
      moveL = [];
      for sL = PnearSL
        sLp = setdiffi(sL.p, IF.p(pID,:));
        for sLpID = 1:size(sLp,1)
          if IF.isin(sLp(sLpID,:))
            continue
          elseif IF.ison(sLp(sLpID,:))
            moveR = sLp(sLpID,:) - IF.p(pID,:);
          else
            moveL = sLp(sLpID,:) - IF.p(pID,:);
          end
        end
      end
      if isempty(moveR) || isempty(moveL)
        warning('moveIF error!');
        break
      end
      IFLp(pID,:) = IF.p(pID,:) + moveL / norm(moveL) * d;
      IFRp(pID,:) = IF.p(pID,:) + moveR / norm(moveR) * d;
    end
    IFR = IF;
    IFL = IF;
    IFR.p = IFRp;
    IFL.p = IFLp;
    IFREN = IF.EN / IF.Area * IFR.Area;
    IFLEN = IF.EN / IF.Area * IFL.Area;
    RwallEN = RwallEN - IF.EN + IFREN;
    LwallEN = LwallEN - IF.EN + IFLEN;
  end
%% plot
